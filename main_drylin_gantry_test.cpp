...
void cb(const geometry_msgs::PoseStamped msg)
{
  ...
  x_ticks = robot.robot.m_pJoints[0].PositionToTicks(qx_ref);   // desired joint (= motor) position in encoder ticks
  y_ticks = robot.robot.m_pJoints[1].PositionToTicks(qy_ref);
